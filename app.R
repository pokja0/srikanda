  ...
  nav_panel(
    ...
    navset_card_underline(
      ...
      nav_panel(
        ...
        h6("Sumber Data: srikandi.arsip.go.id (diakses tanggal 03 Agustus 2026)", style="text-align: center;"),
        layout_columns(
          col_widths = 2,
...
server <- function(input, output, session) {
  ...
  output$tanggal_scraping <- renderText({ "03 Agustus 2026 - Pukul 09.00 WITA"})
  ...
  permintaan_data <- reactive({
    ...
    permintaan_data <- permintaan_data %>%
      mutate(
        # 1. Pastikan semua elemen list diubah menjadi string
        # 2. Tangani format bahasa Indonesia (Februari/Maret)
        `TANGGAL NASKAH` = map_chr(`TANGGAL NASKAH`, function(x) {
          x_str <- as.character(x)
          
          # Jika mengandung nama bulan Indo, konversi menggunakan lubridate
          if (grepl("[A-Za-z]", x_str)) {
            # parse_date_time mendukung locale Indonesia jika diatur, 
            # atau kita bisa lalui dmy() dengan sedikit penyesuaian
            return(as.character(dmy(x_str, locale = "id_ID.utf8")))
          }
          
          # Kembalikan string tanggal standar (YYYY-MM-DD)
          return(substr(x_str, 1, 10))
        }),
        
        # Ubah kembali kolom dari list/character menjadi format Date murni agar seragam
        `TANGGAL NASKAH` = as.Date(`TANGGAL NASKAH`)
      ) |>
      mutate(
        # 1. Pastikan semua elemen list diubah menjadi string
        # 2. Tangani format bahasa Indonesia (Februari/Maret)
        `TANGGAL DEADLINE` = map_chr(`TANGGAL DEADLINE`, function(x) {
          x_str <- as.character(x)
        ...
        # Ubah kembali kolom dari list/character menjadi format Date murni agar seragam
        `TANGGAL DEADLINE` = as.Date(`TANGGAL DEADLINE`)
      ) |>
      mutate(
